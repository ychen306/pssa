...
; RUN: %opt %s -S -passes=test-vector-codegen,adce -p add19,add19.1,add19.2,add19.3/bottom-up -p storeOf:add19,storeOf:add19.1,storeOf:add19.2,storeOf:add19.3 -p i,i2,i5,i7,i10,i12,i15,i17 -p i1,i3,i6,i8,i11,i13,i16,i18 -p i4,i9,i14,i19 -include-pmaddwd=false -include-vpdpwssd=true | FileCheck %s
...
; CHECK-NEXT:    [[BINARY_REDUCER_VEC:%.*]] = call <4 x i32> @llvm.x86.avx512.vpdpwssd.128(<4 x i32> [[I9_VEC]], <4 x i32> [[TMP2]], <4 x i32> [[TMP3]])
