...
; CHECK-NEXT:    br i1 [[CMP18]], label [[TMP0:%.*]], label [[EXIT6:%.*]]
; CHECK:       0:
...
; CHECK-NEXT:    [[TMP4:%.*]] = phi float [ [[DOTPRE]], [[TMP3]] ], [ [[TMP5:%.*]], [[HEADER]] ]
...
; CHECK-NEXT:    [[DOTVEC:%.*]] = load <16 x float>, ptr [[ARRAYIDX]], align 4
...
; CHECK-NEXT:    [[DOTVEC1:%.*]] = load <16 x float>, ptr [[ARRAYIDX4]], align 4
; CHECK-NEXT:    [[TMP5]] = extractelement <16 x float> [[DOTVEC1]], i64 15
; CHECK-NEXT:    [[TMP6:%.*]] = shufflevector <16 x float> [[DOTVEC1]], <16 x float> undef, <16 x i32> <i32 undef, i32 0, i32 1, i32 2, i32 3, i32 4, i32 5, i32 6, i32 7, i32 8, i32 9, i32 10, i32 11, i32 12, i32 13, i32 14>
; CHECK-NEXT:    [[TMP7:%.*]] = insertelement <16 x float> [[TMP6]], float [[TMP4]], i64 0
; CHECK-NEXT:    [[MUL_1_VEC:%.*]] = fmul <16 x float> [[TMP7]], [[DOTVEC]]
; CHECK-NEXT:    store <16 x float> [[MUL_1_VEC]], ptr [[ARRAYIDX2]], align 4
; CHECK-NEXT:    [[ARRAYIDX6:%.*]] = getelementptr inbounds float, ptr [[D:%.*]], i64 [[INDVARS_IV]]
; CHECK-NEXT:    [[DOTVEC2:%.*]] = load <16 x float>, ptr [[ARRAYIDX6]], align 4
; CHECK-NEXT:    [[MUL7_1_VEC:%.*]] = fmul <16 x float> [[DOTVEC2]], [[DOTVEC1]]
; CHECK-NEXT:    [[ARRAYIDX9:%.*]] = getelementptr inbounds float, ptr [[B:%.*]], i64 [[INDVARS_IV]]
; CHECK-NEXT:    [[DOTVEC3:%.*]] = load <16 x float>, ptr [[ARRAYIDX9]], align 4
; CHECK-NEXT:    [[ADD10_1_VEC:%.*]] = fadd <16 x float> [[DOTVEC3]], [[MUL7_1_VEC]]
; CHECK-NEXT:    store <16 x float> [[ADD10_1_VEC]], ptr [[ARRAYIDX9]], align 4
...
; CHECK:       exit:
; CHECK-NEXT:    [[DOTUNR_DEMOTED_0:%.*]] = phi float [ [[DOTPRE]], [[TMP0]] ], [ [[TMP5]], [[HEADER]] ]
; CHECK-NEXT:    [[INDVARS_IV_UNR_DEMOTED_0:%.*]] = phi i64 [ 0, [[TMP0]] ], [ [[INDVARS_IV_NEXT_15]], [[HEADER]] ]
; CHECK-NEXT:    [[LCMP_MOD:%.*]] = icmp ne i64 [[XTRAITER]], 0
; CHECK-NEXT:    br i1 [[LCMP_MOD]], label [[HEADER4:%.*]], label [[EXIT6]]
; CHECK:       header4:
; CHECK-NEXT:    [[EPIL_ITER:%.*]] = phi i64 [ [[EPIL_ITER_NEXT:%.*]], [[HEADER4]] ], [ 0, [[EXIT]] ]
; CHECK-NEXT:    [[INDVARS_IV_EPIL:%.*]] = phi i64 [ [[INDVARS_IV_NEXT_EPIL:%.*]], [[HEADER4]] ], [ [[INDVARS_IV_UNR_DEMOTED_0]], [[EXIT]] ]
; CHECK-NEXT:    [[TMP8:%.*]] = phi float [ [[TMP10:%.*]], [[HEADER4]] ], [ [[DOTUNR_DEMOTED_0]], [[EXIT]] ]
; CHECK-NEXT:    [[ARRAYIDX_EPIL:%.*]] = getelementptr inbounds float, ptr [[C]], i64 [[INDVARS_IV_EPIL]]
; CHECK-NEXT:    [[TMP9:%.*]] = load float, ptr [[ARRAYIDX_EPIL]], align 4, !tbaa [[TBAA5]]
; CHECK-NEXT:    [[ARRAYIDX2_EPIL:%.*]] = getelementptr inbounds float, ptr [[A]], i64 [[INDVARS_IV_EPIL]]
; CHECK-NEXT:    [[MUL_EPIL:%.*]] = fmul fast float [[TMP8]], [[TMP9]]
...
; CHECK-NEXT:    [[TMP10]] = load float, ptr [[ARRAYIDX4_EPIL]], align 4, !tbaa [[TBAA5]]
; CHECK-NEXT:    [[ARRAYIDX6_EPIL:%.*]] = getelementptr inbounds float, ptr [[D]], i64 [[INDVARS_IV_EPIL]]
; CHECK-NEXT:    [[TMP11:%.*]] = load float, ptr [[ARRAYIDX6_EPIL]], align 4, !tbaa [[TBAA5]]
; CHECK-NEXT:    [[MUL7_EPIL:%.*]] = fmul fast float [[TMP11]], [[TMP10]]
; CHECK-NEXT:    [[ARRAYIDX9_EPIL:%.*]] = getelementptr inbounds float, ptr [[B]], i64 [[INDVARS_IV_EPIL]]
; CHECK-NEXT:    [[TMP12:%.*]] = load float, ptr [[ARRAYIDX9_EPIL]], align 4, !tbaa [[TBAA5]]
; CHECK-NEXT:    [[ADD10_EPIL:%.*]] = fadd fast float [[TMP12]], [[MUL7_EPIL]]
...
; CHECK-NEXT:    br i1 [[EPIL_ITER_CMP]], label [[HEADER4]], label [[EXIT6]]
; CHECK:       exit6:
; CHECK-NEXT:    ret void
;
entry:
...
attributes #0 = { argmemonly nofree norecurse nosync nounwind ssp uwtable "approx-func-fp-math"="true" "frame-pointer"="all" "min-legal-vector-width"="0" "no-infs-fp-math"="true" "no-nans-fp-math"="true" "no-signed-zeros-fp-math"="true" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="penryn" "target-features"="+cx16,+cx8,+fxsr,+mmx,+sahf,+sse,+sse2,+sse3,+sse4.1,+ssse3,+x87" "tune-cpu"="generic" "unsafe-fp-math"="true" }
