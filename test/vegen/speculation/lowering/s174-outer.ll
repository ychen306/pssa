...
; RUN: %opt %s -S -passes=test-vector-codegen,simplifycfg -p add,add.1,add.2,add.3 -p storeOf:add,storeOf:add.1,storeOf:add.2,storeOf:add.3 -p i2,i5,i8,i10 -p i3,i6,i9,i11 -find-conditional-deps -remove-redundant-conds | FileCheck %s
...
; CHECK-NEXT:    [[TMP2:%.*]] = add nsw i64 [[TMP1]], 409600000016
